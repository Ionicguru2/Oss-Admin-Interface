...
/**
 * AdminCurrentTransactionCtrl - controller
 */
function CurrentTransactionCtrl($scope, $rootScope, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, API, $modal, $compile, STATUS, TransactionsService) {
    ...
    function approveHtml(data, type, full, meta) {
        ...
        if (data.transaction_stage) {
            ...
            else if (data.transaction_stage.identifier === STATUS.PENDING_VALIDATION) {
              html = '<a ng-click="approveValidation(' + data.id + ')">Approve</a>';
            }
        }
        return html;
    }
    ...
    function denyHtml(data, type, full, meta) {
        ...
        if (data.transaction_stage) {
            if (data.transaction_stage.identifier === STATUS.PENDING_APPROVAL_STATUS) {
              ...
              html = '<a ng-click="denyValidation(' + data.id + ')">Deny</a>';
            }
        }
        return html;
    }

    $scope.approveValidation = function(offer_id) {
        var modalInstance = $modal.open({
            controller: 'BasicTransactionsConfirmModalCtrl',
            templateUrl: 'views/admin/transactions/modal.confirm.approve_validation.html'
        });
        modalInstance.result.then(function(modalResult) {
            TransactionsService.approveValidation(offer_id)
    ...
    $scope.denyValidation = function(offer_id) {
        var modalInstance = $modal.open({
            controller: 'BasicTransactionsConfirmModalCtrl',
            templateUrl: 'views/admin/transactions/modal.confirm.deny_validation.html'
        });
        modalInstance.result.then(function(modalResult) {
            TransactionsService.denyValidation(offer_id)
            .then(function(response) {
                console.log('approve success: ', response);
                $scope.dtInstance.reloadData(function(result) {
                    console.log('reload: ', result);
...
angular
    .module('oss')
    .controller('AdminCurrentTransactionCtrl', CurrentTransactionCtrl);
