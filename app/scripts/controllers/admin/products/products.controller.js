...
function ProductsCtrl($scope, $rootScope, $compile, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, API, $modal, ProductsService, STATUS) {
    ...
    $scope.createProduct = function() {
        var modalInstance = $modal.open({
            controller: 'AdminCreateProductModalCtrl',
            templateUrl: 'views/admin/products/modal.product.create.html'
        });
    ...
    $scope.edit = function(id) {
        if ($scope.products[id].status.name.toLowerCase() == STATUS.PRODUCTION_STATUS_POSTED ||
            ...
            var modalInstance = $modal.open({
                controller: 'AdminEditProductModalCtrl',
                templateUrl: 'views/admin/products/modal.product.edit.html',
                resolve: {
                    userId: function() {
                        return id;
                    }
            ...
            $modal.open({
                controller: 'AdminProductConfirmModalCtrl',
                templateUrl: 'views/admin/products/modal.alert.html'
            });
        }
    };
    ...
    $scope.delete = function(id) {
        var modalInstance = $modal.open({
            controller: 'AdminProductConfirmModalCtrl',
            templateUrl: 'views/admin/products/modal.delete.confirm.html'
        });
        modalInstance.result.then(function(modalResult) {
...
angular
    .module('oss')
    .controller('AdminProductsCtrl', ProductsCtrl);