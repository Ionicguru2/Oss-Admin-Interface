...
function ProductsCtrl($scope, $rootScope, $compile, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, API, $modal, ProductsService, STATUS) {
    ...
    var ajaxOptions = {
        ...
        url: API.base + API.product_company.replace('{id}', $rootScope.loggedInUser.company_id),
        type: 'GET',
    ...
    $scope.createProduct = function() {
        var modalInstance = $modal.open({
            controller: 'PowerCreateProductModalCtrl',
            templateUrl: 'views/power/products/modal.product.create.html'
        });
    ...
    $scope.edit = function(id) {
        if ($scope.products[id].status.name.toLowerCase() == STATUS.PRODUCTION_STATUS_POSTED ||
            ...
            var modalInstance = $modal.open({
                controller: 'PowerEditProductModalCtrl',
                templateUrl: 'views/power/products/modal.product.edit.html',
                resolve: {
                    userId: function() {
                        return id;
                    }
            ...
            $modal.open({
                controller: 'PowerProductConfirmModalCtrl',
                templateUrl: 'views/power/products/modal.alert.html'
            });
        }
    };
    ...
    $scope.delete = function(id) {
        var modalInstance = $modal.open({
            controller: 'PowerProductConfirmModalCtrl',
            templateUrl: 'views/power/products/modal.delete.confirm.html'
        });
        modalInstance.result.then(function(modalResult) {
...
angular
    .module('oss')
    .controller('PowerProductsCtrl', ProductsCtrl);