...
/**
 * PowerCurrentTransactionCtrl - controller
 */
function CurrentTransactionCtrl($scope, $rootScope, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, API, $modal, $compile, STATUS, TransactionsService) {
    ...
    var ajaxOptions = {
        ...
        url: API.base + API.transaction_company + $rootScope.loggedInUser.company_id,
        type: 'GET',
    ...
    $scope.approveTransaction = function(transaction_id) {
        var modalInstance = $modal.open({
            controller: 'PowerTransactionsConfirmModalCtrl',
            templateUrl: 'views/power/transactions/modal.confirm.approve.html'
        });
    ...
    $scope.denyTransaction = function(transaction_id) {
        var modalInstance = $modal.open({
            controller: 'PowerTransactionsConfirmModalCtrl',
            templateUrl: 'views/power/transactions/modal.confirm.deny.html'
        });
        modalInstance.result.then(function(modalResult) {
...
angular
    .module('oss')
    .controller('PowerCurrentTransactionCtrl', CurrentTransactionCtrl);