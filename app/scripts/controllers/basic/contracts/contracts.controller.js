...
function ContractsCtrl($scope, $rootScope, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, API, $modal, $compile, ContractsService) {
    ...
    $scope.openFolder = function(folder_id) {
        ...
        if (folder_id && folder_id !== 1) {
            ...
            endpoint += API.contract_user.replace('{id}', $rootScope.loggedInUser.id);
        }
    ...
    $scope.createFolder = function() {
        var modalInstance = $modal.open({
            controller: 'BasicCreateContractModalCtrl',
            templateUrl: 'views/basic/contracts/modal.contract.create.html',
            resolve: {
                folderId: function() {
    ...
    $scope.edit = function(type, id) {
        /*var modalInstance = $modal.open({
            controller: 'BasicEditContractModalCtrl',
            templateUrl: 'views/basic/contracts/modal.contract.edit.html',
            resolve: {
                type: function() {
                    return type;
                },
    ...
    $scope.delete = function(id) {
        var modalInstance = $modal.open({
            controller: 'BasicContractDeleteConfirmModalCtrl',
            templateUrl: 'views/basic/contracts/modal.delete.confirm.html'
        });
        modalInstance.result.then(function(modalResult) {
    ...
    $scope.uploadFile = function() {
        var modalInstance = $modal.open({
            controller: 'BasicContractUploadModalCtrl',
            templateUrl: 'views/basic/contracts/modal.contract.upload.html'
        });
        modalInstance.result.then(function(modalResult) {
            console.log(modalResult);
        });
...
angular
    .module('oss')
    .controller('BasicContractsCtrl', ContractsCtrl);