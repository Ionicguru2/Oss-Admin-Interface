...
function ProductsCtrl($scope, $rootScope, $compile, DTOptionsBuilder, DTColumnBuilder, DTColumnDefBuilder, API, $modal, ProductsService, STATUS) {
    ...
    $scope.createProduct = function() {
        var modalInstance = $modal.open({
            controller: 'CSRCreateProductModalCtrl',
            templateUrl: 'views/csr/products/modal.product.create.html'
        });
    ...
    $scope.edit = function(id) {
        if ($scope.products[id].status.name.toLowerCase() == STATUS.PRODUCTION_STATUS_POSTED ||
            ...
            var modalInstance = $modal.open({
                controller: 'CSREditProductModalCtrl',
                templateUrl: 'views/csr/products/modal.product.edit.html',
                resolve: {
                    userId: function() {
                        return id;
                    }
            ...
            $modal.open({
                controller: 'CSRProductConfirmModalCtrl',
                templateUrl: 'views/csr/products/modal.alert.html'
            });
        }
    };
    ...
    $scope.delete = function(id) {
        var modalInstance = $modal.open({
            controller: 'CSRProductConfirmModalCtrl',
            templateUrl: 'views/csr/products/modal.delete.confirm.html'
        });
        modalInstance.result.then(function(modalResult) {
...
angular
    .module('oss')
    .controller('CSRProductsCtrl', ProductsCtrl);